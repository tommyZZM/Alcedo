...
(function (alcedo) {
    ...
    (function (async) {
        ...
        async.AsyncRESEvent = AsyncRESEvent;
        ...
        async.AsyncRES = AsyncRES;
    })(async = alcedo.async || (alcedo.async = {}));
})(alcedo || (alcedo = {}));
/**
 * Created by tommyZZM on 2015/5/4.
 */
...
(function (alcedo) {
    ...
    (function (async) {
        ...
        async.RequestMethod = RequestMethod;
        ...
        async.RequestDataType = RequestDataType;
    })(async = alcedo.async || (alcedo.async = {}));
})(alcedo || (alcedo = {}));
/**
 * Created by tommyZZM on 2015/4/8.
...
(function (alcedo) {
    ...
    (function (async) {
        ...
        function ajax(url, args, thisArg) {
            ...
            alcedo.core(AsyncProxy).ajax(url, args, thisArg);
        }
        async.ajax = ajax;
        /**异步载入图片**/
        function asyncImage(url, args, thisArg) {
            alcedo.core(AsyncProxy).asyncImage(url, args, thisArg);
        }
        async.asyncImage = asyncImage;
        var AsyncProxy = (function (_super) {
            ...
            AsyncProxy.prototype.ajax = function (url, args, thisArg) {
                ...
                if (args.responseType == async.RequestDataType.BINARY) {
                    xhr.responseType = async.RequestDataType.BINARY;
                }
                if (args.responseType == async.RequestDataType.BINARY) {
                    ...
                    if (xhr.readyState == 4) {
                        if (xhr.status == 200) {
                            ...
                            switch (args.responseType) {
                                default:
                                case async.RequestDataType.TEXT: {
                                    data = xhr.responseText;
                                    break;
                                }
                                case async.RequestDataType.JSON: {
                                    //TODO:tryCatch隔离
                                ...
                                case async.RequestDataType.BINARY: {
                                    data = xhr.response;
                                    break;
                                }
                            }
                ...
                if (args.method == async.RequestMethod.GET || !args.data) {
                    xhr.send();
                }
            };
    ...
    (function (async) {
        var AsyncAssetsLoader = (function (_super) {
            ...
            AsyncAssetsLoader.prototype.loadAssets = function (assets, groupname, basedir) {
                ...
                if (assets && Array.isArray(assets)) {
                    ...
                    for (var i = 0; i < assets.length; i++) {
                        var asset = assets[i];
                        var name = asset.name;
                        if (alcedo.core(async.AsyncRES).get(name)) {
                            continue;
                        }
            ...
            AsyncAssetsLoader.prototype.loadAsset = function (asset, groupname, basedir) {
                ...
                switch (asset.type) {
                    default:
                    case async.DataType.IMAGE:
                        {
                            async.asyncImage(basedir + "/" + asset.url, {
                                success: function (image, courier) {
                                    alcedo.core(async.AsyncRES).set(courier.name, image); //{type:asset.type,res:image}
                        ...
                        break;
                    case async.DataType.JSON: {
                        async.ajax(basedir + "/" + asset.url, {
                            success: function (json, courier) {
                                ...
                                alcedo.core(async.AsyncRES).set(courier.name, _jsonobj); //{type:asset.type,res:image}
                                _this._oneAsssetComplete(groupname);
                            },
                            error: function () {
                                _this._oneAsssetComplete(groupname);
            ...
            AsyncAssetsLoader.prototype._oneAsssetsGroupComplete = function () {
                ...
                if (flag)
                    this.emit(async.AsyncRESEvent.ASSETS_COMPLETE);
            };
            AsyncAssetsLoader.instanceable = true;
            return AsyncAssetsLoader;
        })(alcedo.AppSubCore);
        async.AsyncAssetsLoader = AsyncAssetsLoader;
    })(async = alcedo.async || (alcedo.async = {}));
})(alcedo || (alcedo = {}));
/**
 * Created by tommyZZM on 2015/4/6.
        ...
        var ParticleEmitter = (function (_super) {
            ...
            ParticleEmitter.prototype._createOneParticle = function () {
                ...
                partile.onDecay(function (particle) {
                    ...
                    var index = _this._particles.indexOf(particle);
                    var result = _this[index];
                    if (_this._particles.length == 1) {
                        _this._particles.length = 0;
                    }
                    _this[index] = _this._particles.pop();
        ...
        var TextureRepository = (function (_super) {
            ...
            TextureRepository.prototype.get = function (key) {
                ...
                if (!this._texurespool.has(key)) {
                    if (alcedo.core(alcedo.async.AsyncRES).get(key) && alcedo.core(alcedo.async.AsyncRES).get(key)[0] instanceof HTMLImageElement) {
                        var img = alcedo.core(alcedo.async.AsyncRES).get(key)[0];
            ...
            TextureRepository.prototype.find = function (reg) {
                var i, keys = alcedo.core(alcedo.async.AsyncRES).keys, result = [];
                for (i = 0; i < keys.length; i++) {
                    if (reg.test(keys[i])) {
                        if (this.get(keys[i]))
                            result.push(this.get(keys[i]));
